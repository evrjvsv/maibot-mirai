...
from nonebot.adapters.mirai2 import MessageEvent as Event, Bot, FriendMessage, MessageChain as Message
...
def song_txt(music: Music):
    return Message([
        ...
        },
        {
            ...
            "url": f"https://www.diving-fish.com/covers/{music.id}.jpg"
        },
        {
            "type": MessageType.PLAIN,
            "text": f"\n{'/'.join(music.level)}"
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    if len(result_set) == 1:
        ...
        await find_song.finish(Message([{"type": MessageType.PLAIN, "text": "您要找的是不是"}]+song_txt(music)))
    else:
        s = '\n'.join(result_set)
...
@best_40_pic.handle()
async def _(bot: Bot, event: Event, state: T_State):
    username = str(event.get_message()).strip("/b40").strip()
    if username == "":
    ...
    if success == 400:
        ...
        await best_40_pic.send(Message([
            {
                "type": MessageType.IMAGE,
                "base64": f"{str(image_to_base64(img), encoding='utf-8')}"
            }
        ]))
...
@best_50_pic.handle()
async def _(bot: Bot, event: Event, state: T_State):
    username = str(event.get_message()).strip("/b50").strip()
    if username == "":
